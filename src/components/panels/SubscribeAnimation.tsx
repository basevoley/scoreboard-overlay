...
const SubscribeAnimation = ({ config }: SubscribeAnimationProps) => {
  ...
  useEffect(() => {
    if (enabled) launchSocialConfetti();
  }, [enabled]);
    ...
    <div ref={containerRef} className={`${styles['overlay-wrapper']} ${positionClass} ${enabled ? styles.visible : styles.hidden}`}>
      <div className={styles.overlayContainer}>
        ...
        <motion.div
          ...
          transition={{ duration: animTime, repeat: Infinity, repeatDelay: cycleTime - animTime, delay: 0.7 }}
        >
        ...
        <motion.div
          ...
          transition={{ duration: animTime, repeat: Infinity, repeatDelay: cycleTime - animTime, delay: 2.1 }}
        >
          <svg viewBox="0 0 24 24" style={{ height: '64px', width: '64px', color: 'whitesmoke' }}>
